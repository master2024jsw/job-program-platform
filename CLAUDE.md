...
- **진행 상태** (2026-09-21 기준, 05문서 7절 체크리스트와 동일)
  - [x] A. 사업유형·서류유형·필수서류 데이터 + API
  - [x] B. 로그인 화면 사업 드롭다운
  - [ ] C1. businessId 스코프 — 백엔드 (다음 작업)
  - [ ] C2. businessId 스코프 — 프론트엔드
  - [ ] D. 서류유형 코드 적용
  - [ ] E. 필수서류 체크리스트 UI
- **다음 세션에서 바로 할 일**: C1단계("5개 엔티티 businessId, 권한 가드, SubsidySetting 자동 생성, 메일수집 사업 매칭") 변경안을 세워 승인받고 진행 — 05문서 4·5절에 파일 목록·커밋 단위가 정리되어 있음
...
- 2026-09-21: 논문 심사 지적 대응 기준 문서(01, 04) 반영, 05 실행계획 수립 후 A·B단계 완료 (사업유형 3종·서류유형 코드·필수서류 API, 로그인 화면 사업 선택) — 진행 중, 상세는 위 "진행 중인 작업" 참고
