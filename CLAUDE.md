...
- **진행 상태** (2026-09-22 기준, 05문서 7절 체크리스트와 동일)
  ...
  - [ ] C2. businessId 스코프 — 프론트엔드 (다음 작업)
  - [ ] D. 서류유형 코드 적용
  - [ ] E. 필수서류 체크리스트 UI
- **다음 세션에서 바로 할 일**: C2단계(프론트 화면을 현재 사업 기준으로 연동, 문서함 미분류 탭) 진행 — 05문서 4·5절에 파일 목록·커밋 단위가 정리되어 있음. C1은 같은 세션에서 이어서(중간에 앱이 깨진 상태로 두지 않기) 진행하는 게 원칙이었으나 이번엔 백엔드만 우선 완료.
...
- 2026-09-22: 05 실행계획 C1단계 완료 (문서·근로자·메일·지원금 데이터 businessId 스코프 + 권한 가드, 백엔드만) — 진행 중, 상세는 위 "진행 중인 작업" 참고
